...
int Smart_IO_main(void)
    {
      // Setup();
      // Smart-IO call to set up clock, systick, and then uart2,
      // then print to console sysclk frequency
      // STM32CubeMX HAL calls do the system init:
      printf("\r\nImageCraft STM32F407 Discovery Board... System running at %dMhz\n", SystemCoreClock);
    ...
    while (1)
        ...
        uint8_t cwbuf[] = (uint8_t)"While Smart-IO is alive...\r\n";
        HAL_UART_Transmit(&huart3, cwbuf, 29, 1000);

        if (something_happened)
            {
            Smart_IO_putchar('>'); Smart_IO_putchar(' ');
            something_happened = 0;
            }
        if //(SPI_State == SPI_IDLE && usart2.kbhit())
          // use huart3 amd HAL
          (SPI_State == SPI_IDLE && huart3.RxXferCount)
            {
...
int Smart_IO_putchar( int ch)
//int Smart_IO_putchar( uint8_t ch)
    {
...
static void Setup(void)
    {
/*      
// Set up clock and systick, then UART2, then print to console sysclk frequency
    jsapi_clock.SetSystemClock(16, 0, false, 84, 5);
    jsapi_cortex_core.SysTick_Timer(SYSTICK_MILLISECOND);

    usart2.SetPins(&porta, 2, 7, &porta, 3, 7); //Whats this for?
    usart2.MakeUSART(9600, 8, 1, 0);    //Com from host to Smart-io
*/
    printf("\r\nImageCraft STM32F407 Discovery Board... System running at %dMhz\n", SystemCoreClock);
