...
#include "LibSmartIO-AVR-M328P-JumpStartC/smartio_interface.h"
...
// 0b10 char *
// 0b11 uint32_t
#define _1ARG       0x01        //0000000000000001
#define _2ARGS      0x05        //0000000000000101
#define _3ARGS      0x15        //0000000000010101
#define _4ARGS      0x55        //0000000001010101
#define _5ARGS      0x155       //0000000101010101
#define _6ARGS      0x555       //0000010101010101
#define _7ARGS      0x1555      //0001010101010101
#define _8ARGS      0x5555      //0101010101010101

#define STRARG(i)   (0x02 << (i-1)*2)
...
//    return APICall(0b01111101010101, 10, alignment, 310, start_color, end_color, initial_percentage);
    return APICall(0x1f55, 10, alignment, 310, start_color, end_color, initial_percentage);
    }
...
//    return APICall(0b01111101010101, 10, alignment, 311, start_color, end_color, initial_percentage);
    return APICall(0x1f55, 10, alignment, 311, start_color, end_color, initial_percentage);
    }
...
static uint32_t APICall(unsigned argmask, ...)
    ...
    for (int i = 0; i < 8; i++)
        {
        unsigned mask = argmask & 0x3; //0b11
        if (mask == 0)
            break;
        if (mask == 0x01)
            {
        ...
        else if (mask == 0x02 && Command[0] == SMARTIO_SYS_EEPROM_WRITE)
            {
            if (index != 3)
                return (uint32_t)-1;
